-- Dedupe staging (keep last row per CPF)
DELETE FROM public._stg_sync_prof a
USING public._stg_sync_prof b
WHERE a.ctid < b.ctid AND a.cpf = b.cpf;

-- Phase A: rename existing matriculas to temp to avoid unique collisions
UPDATE public.profissionais
SET matricula = '__tmp4__' || id::text
WHERE tenant_id = '00000000-0000-0000-0000-000000000001';

-- Phase B: UPDATE from staging (planilha as source of truth)
...
-- Phase D: alertas para os 6 profissionais sem correspondência na planilha
INSERT INTO public.alertas_sistema (tenant_id, tipo, prioridade, titulo, mensagem, profissional_id)
SELECT '00000000-0000-0000-0000-000000000001', 'dado_faltante', 'alta',
       'Profissional sem correspondência na planilha de ativos: ' || nome,
       'Esse profissional existia no sistema mas não consta na planilha de ATIVOS enviada. Revisar manualmente.', id
FROM public.profissionais
WHERE tenant_id = '00000000-0000-0000-0000-000000000001'
  AND matricula LIKE '__tmp4__%';

-- Phase E: corrige matriculas dos órfãos
UPDATE public.profissionais
SET matricula = 'PEND-' || substr(id::text, 1, 8)
WHERE tenant_id = '00000000-0000-0000-0000-000000000001'
  AND matricula LIKE '__tmp4__%';

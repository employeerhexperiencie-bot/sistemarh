  ...
  const escapeLikePattern = (str: string): string => {
    ...
    if (profissionais && profissionais.length > 0) {
      for (const prof of profissionais as ExcelProfissional[]) {
        ...
        const cpfStr = prof.cpf && String(prof.cpf).trim() ? String(prof.cpf).trim() : '';
        const rgStr = prof.rg && String(prof.rg).trim() ? String(prof.rg).trim() : '';
        const pisStr = prof.pis && String(prof.pis).trim() ? String(prof.pis).trim() : '';
        const ctpsStr = prof.ctps && String(prof.ctps).trim() ? String(prof.ctps).trim() : '';
